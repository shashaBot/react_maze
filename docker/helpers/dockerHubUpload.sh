...
then
  docker tag maze-game-image-prod sandunwebdev/maze-game-image-prod:master;

  docker push sandunwebdev/maze-game-image-prod:master;
elif [ "$TRAVIS_BRANCH" == "release" ] && [ "$TRAVIS_PULL_REQUEST" == "false" ]
then
  docker tag maze-game-image-prod sandunwebdev/maze-game-image-prod:release;
  docker tag maze-game-image-prod sandunwebdev/maze-game-image-prod:latest;

  docker push sandunwebdev/maze-game-image-prod:release;
  docker push sandunwebdev/maze-game-image-prod:latest;
fi
